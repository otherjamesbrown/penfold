...
func TestTimeoutConfig_LoadFromDB(t *testing.T) {
	...
	// Verify values match what's in pipeline_config table
	expectedKeys := []string{
...
func TestTimeoutConfig_SetAndRefresh(t *testing.T) {
	...
	t.Cleanup(func() {
		_, _ = db.Pool.Exec(ctx,
			"UPDATE pipeline_config SET value = $1, updated_at = now(), updated_by = 'test_cleanup' WHERE key = $2",
...
func TestTimeoutConfig_OnChangeCallback(t *testing.T) {
	...
	_, err = db.Pool.Exec(ctx,
		"UPDATE pipeline_config SET value = $1, updated_at = now(), updated_by = 'test' WHERE key = $2",
		newDuration.String(), testKey,
	)
	require.NoError(t, err)
...
func TestTimeoutConfig_DefaultsFallback(t *testing.T) {
	...
	for _, key := range keys {
		...
		// Read DB default_value column to compare with hardcoded
		var dbDefaultStr string
		err := db.Pool.QueryRow(ctx,
			"SELECT default_value FROM pipeline_config WHERE key = $1", key,
		).Scan(&dbDefaultStr)
		require.NoError(t, err, "key %s should have default_value in DB", key)

		dbDefaultVal, err := time.ParseDuration(dbDefaultStr)
		require.NoError(t, err)

		assert.Equal(t, dbDefaultVal, defaultVal,
			"key %s: hardcoded default %v should match DB default_value %v (drift detected)", key, defaultVal, dbDefaultVal)
