// Package observability provides shared metrics, tracing, and logging interceptors for Temporal workflows.
// It integrates with Prometheus for metrics, OpenTelemetry for tracing, and zerolog for structured logging.
...
type Metrics struct {
	...
	// mu protects concurrent access during registration
	mu sync.Mutex
}
...
func (m *Metrics) initMetrics() {
	...
	// Task queue metrics
	m.TaskQueueDepth = prometheus.NewGaugeVec(
		prometheus.GaugeOpts{
			Namespace: m.config.Namespace,
			Subsystem: "temporal",
			Name:      "task_queue_depth",
			Help:      "Estimated depth of task queue (pending tasks)",
			ConstLabels: prometheus.Labels{
				"service": m.config.ServiceName,
			},
		},
...
// SetTaskQueueDepth sets the estimated task queue depth.
func (m *Metrics) SetTaskQueueDepth(taskQueue, taskType string, depth float64) {
	m.TaskQueueDepth.WithLabelValues(taskQueue, taskType).Set(depth)
}
...
// contains is a simple string contains check (avoids importing strings package).
