# Task: Add model-aware semaphore map to AI coordinator
...
## Phase 3a: Model-aware semaphores — config + semaphore infrastructure

### Description
Add per-backend concurrency semaphores to the AI coordinator. This task builds the semaphore map, config loading from `pipeline_operational_config`, startup validation, and periodic hot-reload. The next task (3b) wires acquire/release into the RPC handlers.

### Changes

**1. Semaphore map and config loading in composite.go**

File: `services/ai/backend/composite.go`

The `CompositeBackend` struct already has the provider routing logic (`extractProvider()` at line 36-52). Add:
...
type CompositeBackend struct {
    ...
    configLoader      func() (map[string]int, error) // reads from DB
}
```

**2. Config loading function**

Add a function that reads `model.concurrency.*` keys from `pipeline_operational_config`:
- Keys: `model.concurrency.ollama`, `model.concurrency.gemini`, `model.concurrency.anthropic`, `model.concurrency.mlx`, `model.concurrency.default`
- Provider strings match `extractProvider()` return values: "ollama", "gemini", "anthropic", "mlx"
- If `model.concurrency.default` is missing at startup, return an error (startup validation — fail visibly)

**3. Semaphore initialization**

On startup, load config and create buffered channels:
```go
func (c *CompositeBackend) initSemaphores() error {
    config, err := c.configLoader()
    // For each provider, create chan struct{} with buffer size = max concurrent
    // Store in backendSemaphores map
}
```

**4. Hot reload (periodic refresh)**

Every 60 seconds, reload config. If a limit changed, resize by creating a new channel and draining/filling as needed. Use a background goroutine started at init.

**5. Acquire/Release helpers**

```go
func (c *CompositeBackend) acquireSemaphore(ctx context.Context, provider string) error {
    sem := c.getSemaphore(provider) // falls back to "default"
    select {
    case sem <- struct{}{}:
        return nil
    case <-ctx.Done():
...
func (c *CompositeBackend) releaseSemaphore(provider string) {
    sem := c.getSemaphore(provider)
    <-sem
}
```

### Acceptance Criteria
- [ ] CompositeBackend has backendSemaphores map initialized from DB config
- [ ] Missing `model.concurrency.default` key raises startup error
- [ ] acquireSemaphore blocks when at capacity, respects context cancellation
- [ ] releaseSemaphore frees a slot
- [ ] Config reloads every 60s and resizes semaphores
- [ ] Unit tests cover: acquire/release, context cancellation, fallback to default, missing config error

### Files Changed (max 3)
1. `services/ai/backend/composite.go` (edit — add semaphore fields, init, acquire/release)
2. `services/ai/backend/composite_test.go` (new or edit — unit tests)
3. `services/ai/backend/config.go` (new — config loading if composite.go gets too large)

### Dependencies
- Task pf-5feb30 (migration 151 seeds the config rows)
...
1. **Run `cobuild complete pf-88e86e`** -- this commits remaining changes, pushes, creates the PR, appends evidence, and marks the task needs-review. Do this as your LAST action.
