# Task: Add queue backpressure with dynamic ScheduleToClose timeout
...
## Phase 4: Queue backpressure — dynamic ScheduleToClose

### Description
When the pending queue is deep, extend the ScheduleToClose timeout for new workflows to prevent queued workflows from timing out while waiting for a slot. This is a safety net after Phases 1-3 reduce queue buildup.

### Changes

**1. Dynamic ScheduleToClose in gateway**

File: `services/gateway/pipelineservice/service.go` — in the `startWorkflow` area

Before starting a new Temporal workflow, read the current pending count and adjust ScheduleToClose:

```go
scheduleToClose := 1 * time.Hour // default
if pendingCount > 100 {
    scheduleToClose = 4 * time.Hour
} else if pendingCount > 50 {
    scheduleToClose = 2 * time.Hour
}
```

The pending count is already available or easily queryable from the pipeline repository (used by KickNextPending slot calculation).

**2. Log the adjustment**

When ScheduleToClose is extended beyond default, log at INFO level:
```
pipeline.schedule_to_close_extended pending_count=157 timeout=4h
```

### Acceptance Criteria
- [ ] With <50 pending items, ScheduleToClose remains at default (1h)
- [ ] With 50-100 pending items, ScheduleToClose extends to 2h
- [ ] With >100 pending items, ScheduleToClose extends to 4h
- [ ] Extension is logged at INFO level with pending count
- [ ] No change to existing workflow behaviour when queue is shallow

### Files Changed (max 2)
1. `services/gateway/pipelineservice/service.go` (edit — add pending count check before workflow start)
2. `services/gateway/pipelineservice/service_test.go` (edit — test backpressure logic)

### Dependencies
- Independent, but should be deployed after Phases 1-3 to assess whether it's needed
...
1. **Run `cobuild complete pf-aa6fcb`** -- this commits remaining changes, pushes, creates the PR, appends evidence, and marks the task needs-review. Do this as your LAST action.
