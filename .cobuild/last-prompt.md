# Task: Fix: Add cleanup and dedup handling to TestEval_Newsletter
...
Bug pf-362e49 (secondary issue): All 6 newsletter eval test emails return source_id 3818, indicating deduplication against stale sources from previous test runs.

## Root Cause

The .eml fixture files have static Message-ID headers. On repeated runs, `CheckDuplicate()` in the ingest service matches on message_id and returns the original source_id instead of creating a new source. The test generates unique source_tags but dedup fires before source creation.

Compare with `TestQuality_ExtractionAccuracy` which calls `CleanupTestTenant()` before ingesting — this deletes old sources so dedup doesn't trigger.

## Required Changes

In `penfold/tests/quality/newsletter_eval_test.go`:

1. Call `env.CleanupTestTenant()` at the start of the test (before the golden file loop) to remove stale sources from prior runs. This eliminates message_id dedup collisions.

2. Alternatively, call `env.LoadFixtures()` which implicitly calls `EnsureTenantExists()` — but cleanup is still needed to remove old sources.

## Acceptance Criteria

- [ ] Each newsletter test email gets a unique source_id (no dedup collision)
- [ ] Test is safe to re-run without manual DB cleanup
- [ ] source_id values differ across the 6 newsletter emails

## Files to Modify

- `penfold/tests/quality/newsletter_eval_test.go` — add CleanupTestTenant() call at start
...
2. Build: `make build`
3. **Run `cobuild complete pf-1da1da`** -- this commits remaining changes, pushes, creates the PR, appends evidence, and marks the task needs-review. Do this as your LAST action.
