-- Migration 007: Tenant Configuration Tables
-- Part of Content Enrichment Pipeline - Configuration & API (pe-vcz1)
...
COMMENT ON TABLE tenants IS 'Tenant organizations for multi-tenant support';
COMMENT ON COLUMN tenants.slug IS 'URL-safe identifier used in paths and references';
COMMENT ON COLUMN tenants.settings IS 'Tenant-wide settings (JSON)';
...
CREATE INDEX idx_tenant_email_patterns_tenant ON tenant_email_patterns(tenant_id);
CREATE INDEX idx_tenant_email_patterns_enabled ON tenant_email_patterns(tenant_id, enabled) WHERE enabled = true;
...
CREATE INDEX idx_tenant_integrations_enabled ON tenant_integrations(tenant_id, enabled) WHERE enabled = true;
...
CREATE INDEX idx_tenant_jira_mappings_integration ON tenant_jira_mappings(integration_id);
CREATE INDEX idx_tenant_jira_mappings_project ON tenant_jira_mappings(penfold_project_id);
...
CREATE INDEX idx_tenant_processing_rules_tenant ON tenant_processing_rules(tenant_id);
CREATE INDEX idx_tenant_processing_rules_enabled ON tenant_processing_rules(tenant_id, enabled, priority) WHERE enabled = true;
...
VALUES ('Default', 'default', 'Default tenant for single-tenant deployment', true)
