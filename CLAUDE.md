# Task: Eval Phase 2 — extend golden YAML types for notification category
...
## Scope
Extend tests/quality/types.go with notification-specific type definitions. This mirrors the Phase 1 pattern (pf-a21ebd) for newsletters but adds notification-specific fields.

### New/Extended Types

1. **NotificationExtractExpectation** — validates notification-specific extraction output:
   - `handling_mode`: one of `immediate_escalate`, `triage_once`, `daily_summary`, `compliance_status`
   - `notification_source`: expected source string (aha, jira, google, oracle, globalsecops, etc.)
   - `is_repeat`: bool — whether this is a repeat reminder (for triage_once suppression logic)
   - `escalation`: *EscalationExpectation — for immediate_escalate items only
   - `compliance`: *ComplianceExpectation — for compliance_status items only
   - `activity_summary`: *StringFieldExpectation — for daily_summary items (aggregation description)

2. **EscalationExpectation** — validates immediate_escalate extraction:
   - `threat_description`: *StringFieldExpectation (must_mention, min_length)
   - `urgency`: *OneOfMatcher
   - `requires_response`: *bool

3. **ComplianceExpectation** — validates compliance_status extraction:
   - `course_name`: *StringFieldExpectation
   - `assignees`: *ItemListExpectation (people assigned)
   - `due_date_present`: *bool
   - `overdue`: *bool

4. **ActualNotificationExtraction** — represents parsed notification extraction output from DB:
   - Maps to whatever JSON schema the notification pipeline stores in content_enrichment

5. Extend **RoutingExpectation** — already has `notification_source` field, verify it works for notifications

### Acceptance Criteria
- [ ] NotificationExtractExpectation struct defined with all handling-mode-specific sub-structs
- [ ] EscalationExpectation and ComplianceExpectation defined
- [ ] ActualNotificationExtraction struct defined matching DB schema
- [ ] Package compiles: `go build -tags=quality ./tests/quality/...`
- [ ] Existing golden YAML files parse without error (backward compatible)
- [ ] New types support all 4 handling modes from the notification intent spec

### Code Locations
- tests/quality/types.go — ALL changes go here, append after existing newsletter types

### Approach
Follow exact pattern from Phase 1 (pf-a21ebd). All types are pointer/omitempty for backward compat. Check the notification pipeline's actual DB schema (content_enrichment table, pipeline_runs) to ensure ActualNotificationExtraction matches reality.

### Dependencies
None — types only, no matcher logic.
...
3. **Run `cxp task complete pf-20233c`** — this commits remaining changes, pushes, creates the PR, appends evidence, and marks the task needs-review. Do this as your LAST action.
