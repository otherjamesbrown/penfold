# Task: Eval Phase 2 — notification triage calibration and handling mode matchers
...
## Scope
Implement assertion matchers for notification-specific eval checks. This covers L1 routing (reuses existing RoutingExpectation) and L2 quality matchers for notification handling modes and triage calibration.

### New Matchers to Implement

1. **MatchNotificationExtract(t, expected, actual)** — top-level dispatcher:
   - Validates handling_mode matches expected
   - Validates notification_source matches expected
   - Dispatches to handling-mode-specific sub-matchers

2. **matchTriageOnce(t, expected, actual)** — for triage_once items:
   - Validates is_repeat detection (new vs repeat reminder)
   - Validates task description present
   - Validates assignee/due date extraction

3. **matchDailySummary(t, expected, actual)** — for daily_summary items:
   - Validates activity_summary field present and meets min_length
   - Validates project context extraction

4. **matchComplianceStatus(t, expected, actual)** — for compliance_status items:
   - Validates course_name extraction
   - Validates assignees list (person + due date)
   - Validates overdue flag

5. **matchImmediateEscalate(t, expected, actual)** — for immediate_escalate items:
   - Validates threat_description present and meets must_mention
   - Validates urgency level
   - Validates requires_response flag

6. **Extend matchOneOf** to enforce MustNotBe — the OneOfMatcher already has the field, but the matcher in matchers.go does not check it yet.

### EvalResults Integration
All matchers must return MatchDetail records for Langfuse scoring. Follow the existing pattern where matchers append to EvalResults.L1Routing or L2Quality.

### Acceptance Criteria
- [ ] MatchNotificationExtract function dispatches to 4 handling-mode sub-matchers
- [ ] MustNotBe enforcement added to matchOneOf
- [ ] Each handling mode has at least 3 assertion checks
- [ ] All matchers produce MatchDetail records for EvalResults
- [ ] Package compiles: `go build -tags=quality ./tests/quality/...`
- [ ] Unit tests for each matcher with table-driven test cases

### Code Locations
- tests/quality/notification_matchers.go — NEW file, all notification-specific matchers
- tests/quality/matchers.go — extend matchOneOf with MustNotBe check
- tests/quality/notification_matchers_test.go — NEW file, unit tests

### Approach
Follow the pattern from matchers.go (MatchPeople, MatchAssertions, etc.). Each matcher takes *testing.T, expected expectation, and actual extraction data. Use EvalResults accumulation pattern from Phase 1. The MustNotBe extension to matchOneOf is a small change that benefits all categories.

### Dependencies
- pf-20233c (notification types must exist)
...
3. **Run `cxp task complete pf-b8ed97`** — this commits remaining changes, pushes, creates the PR, appends evidence, and marks the task needs-review. Do this as your LAST action.
